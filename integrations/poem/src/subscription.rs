    ...
    OnConnInitFut: Future<Output = async_graphql::Result<Data>> + Send + 'static,
{
    ...
    #[must_use]
    pub fn per_message_post_hook<F, Fut>(self, hook: F) -> Self
    where
        Fut: Future<Output = async_graphql::Result<()>> + Send + 'static,
        F: Fn() -> Fut + Send + Sync + 'static,
    {
        GraphQLWebSocket {
            sink: self.sink,
            ...
            protocol: self.protocol,
            per_message_pre_hook: self.per_message_pre_hook,
            per_message_post_hook: Some(Arc::new(move || Box::pin(hook()))),
        }
    }
